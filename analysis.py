...
users = pd.DataFrame(json.load(open("clean_users.json")))
...
latest_date = history["timestamp"].max()

start_date = latest_date - pd.Timedelta(days=30)

recent_history = history[
    history["timestamp"].between(
        start_date,
        latest_date
    )
].copy()
...
print(
    "\nUser ratings over the most recent 30 days:"
)
...
most_watched_genre = (
    genre_views.iloc[0]["genre"]
)

most_watched_genre_views = (
    genre_views.iloc[0]["view_count"]
)

highest_rated_genre = (
    genre_user_ratings.iloc[0]["genre"]
)

highest_rated_genre_score = (
    genre_user_ratings.iloc[0][
        "average_user_rating"
    ]
)

most_popular_movie = (
    most_watched_movies.iloc[0]["movie_id"]
)

most_popular_movie_views = (
    most_watched_movies.iloc[0]["view_count"]
)

highest_rated_movie = (
    highest_rated_movies.iloc[0]["movie_id"]
)

highest_rated_movie_score = (
    highest_rated_movies.iloc[0]["rating"]
)

most_active_user = (
    most_active_users.iloc[0]["user_id"]
)

most_active_user_views = (
    most_active_users.iloc[0]["total_views"]
)

print(
    f"1. The most watched genre is "
    f"{most_watched_genre} with "
    f"{most_watched_genre_views} "
    f"viewing events."
)

print(
    f"2. The genre with the highest "
    f"average user rating is "
    f"{highest_rated_genre} with a "
    f"rating of "
    f"{highest_rated_genre_score}/5."
)

print(
    f"3. The most watched movie is "
    f"Movie {most_popular_movie} with "
    f"{most_popular_movie_views} "
    f"viewing events."
)

print(
    f"4. The highest-rated movie in "
    f"the movie dataset is Movie "
    f"{highest_rated_movie} with a "
    f"rating of "
    f"{highest_rated_movie_score}/10."
)

print(
    f"5. The most active user is "
    f"User {most_active_user} with "
    f"{most_active_user_views} "
    f"viewing events."
)
...
genre_views.to_csv(
    "analysis_genre_views.csv",
    index=False
)

genre_user_ratings.to_csv(
    "analysis_genre_ratings.csv",
    index=False
)
...
movie_performance.to_csv(
    "analysis_movie_performance.csv",
    index=False
)

age_analysis.to_csv(
    "analysis_age_groups.csv"
)

rating_over_time.to_csv(
    "analysis_rating_over_time.csv",
    index=False
)
