...
viewing_history = pd.read_csv(BASE_DIR / "viewing_history.csv")
...
for user in users:
    ...
    preferences = [
        genre
        for genre in (user.get("preferences") or [])
        if genre in valid_genres
    ]

    watch_history = []

    for watch in (user.get("watch_history") or []):
...
processed_movies.to_csv(
    BASE_DIR / "processed_movies.csv",
    index=False
)

viewing_history.to_csv(
    BASE_DIR / "clean_viewing_history.csv",
    index=False
)

user_stats.to_csv(
    BASE_DIR / "user_stats.csv",
    index=False
)

user_movie_data.to_csv(
    BASE_DIR / "user_movie_data.csv",
    index=False
)

with open(BASE_DIR / "clean_users.json", "w", encoding="utf-8") as file:
